...
| Variable args   | =str_match_named_short= | =str_match_all_named_short= |
...
The namedCapture package also provides =str_match*short= functions
...
The =str_match_named_short= code below yields the same result as the
...
The =str_match_all_named_short= code below yields a data.frame with
one row for each match in the subject, and one column for each named
...
The example below illustrates two features of namedCapture functions
that provide named output:
...
element is a data.frame with rownames defined by the =name= capture
