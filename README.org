...
| Package | First match | All matches | Comments |
|---------|-------------|-------------|----------|
| namedCapture | str_match_named | str_match_all_named | Character vector subject, character string pattern |
| namedCapture | str_match_variable | str_match_all_variable | Character vector subject, list/character/function pattern |
| namedCapture | df_match_variable | NA | Data frame subject, list/character/function patterns |
| rex     | re_matches(global=FALSE) | re_matches(global=TRUE) | verbose/expressive regex construction |
| base    | regexpr     | gregexpr    | outputs group indices/names |
| utils | strcapture | NA | outputs data frame |
| stringr | str_match | str_match_all | named capture not supported |
| stringi | stri_match | stri_match_all | named capture not supported |
| tidyr | extract | NA | named capture not supported |
| rematch2 | TODO |
| re2r | TODO |
...
#+BEGIN_SRC R
  ...
  ## Example 1: extract all fields for each track.
  ...
  ## Example 2: extract only the bigDataUrl field for each track, and
  ## split sample names into separate columns. Also demonstrates use of
