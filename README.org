...
- The two packages can be used to compute the same result. (a
  ...
  columns)
- The =tidyr= code is about twice as long (33 lines) as the
  =namedCapture= code (17 lines). This is mostly because the for loop
  that you see below for =tidyr= is hidden inside the definition of
  =namedCapture::df_match_variable=.
- The =tidyr= code defines =regex.list= which uses a syntax similar to
