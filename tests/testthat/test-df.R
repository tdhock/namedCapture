...
range.pattern <- list(
  ...
  ")?", #end is optional.
  "[]]")
test_that("df_match_variable returns data.frame with rownames", {
...
no.rownames <- data.frame(
  ...
  stringsAsFactors=FALSE)
test_that("df_match_variable returns data.frame with default rownames", {
  match.df <- df_match_variable(
    no.rownames,
    JobID=list(
      job="[0-9]+", as.integer,
...
test_that("df_match_variable takes rownames from first pattern", {
  match.df <- df_match_variable(
    uniq.chr,
    JobID=list(
      name="[^.]+[.].|[0-9]+",
...
test_that("df_match_variable takes rownames from second pattern", {
  match.df <- df_match_variable(
    uniq.chr,
    JobID=list(
      pre="[^.]+[.].|[0-9]+",
...
named.uniq.chr <- data.frame(
  ...
  position=c(
    ...
    "chr1:110-111 chr2:220-222"),
  stringsAsFactors=FALSE)
test_that("df_match_variable does not take rownames from first pattern", {
  match.df <- df_match_variable(
    named.uniq.chr,
    JobID=list(
      name="[^.]+[.].|[0-9]+",
...
test_that("take names from named subject even with two named groups", {
  match.df <- df_match_variable(
    named.uniq.chr,
    JobID=list(
      name="[^.]+[.].|[0-9]+",
